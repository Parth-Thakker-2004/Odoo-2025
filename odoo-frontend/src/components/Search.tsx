...
const Search = () => {
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (!showPrivate) {
      users = users.filter(user => user.profileVisibility === 'public');
    }
    ...
    if (searchTerm) {
      users = users.filter(user => {
        ...
        if (filterType === 'all') {
          ...
          skillMatch = user.skillsOffered.some(skill => 
            skill.toLowerCase().includes(searchLower)
          );
        } else if (filterType === 'wanted') {
          skillMatch = user.skillsWanted.some(skill => 
            skill.toLowerCase().includes(searchLower)
          );
        }
    ...
    return users;
  }, [searchTerm, filterType, showPrivate]);
