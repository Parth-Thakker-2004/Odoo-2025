...
import { User, SwapRequest, currentUserId, demoSwapRequests } from '@/data/demoData';
...
const UserCard = ({ user, showRequestButton = true }: UserCardProps) => {
  ...
  const handleSendRequest = () => {
    ...
    const existingRequest = demoSwapRequests.find(
      req => req.fromUserId === currentUserId && req.toUserId === user.id && req.status === 'pending'
    );
    ...
    <>
      <Card className="w-full max-w-md hover:shadow-lg transition-shadow">
        ...
        {showRequestButton && user.id !== currentUserId && (
          <CardFooter>
            <Button onClick={handleSendRequest} className="w-full" disabled={isPrivate}>
